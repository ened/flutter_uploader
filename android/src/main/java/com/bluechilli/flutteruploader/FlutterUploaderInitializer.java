    ...
    private static int getMaxConcurrentTaskMetadata(Context context) {
        try {
            ProviderInfo pi = context.getPackageManager().getProviderInfo(
                    new ComponentName(context, "com.bluechilli.flutteruploader.FlutterUploaderInitializer"),
                    PackageManager.GET_META_DATA);
            Bundle bundle = pi.metaData;
            int max = bundle.getInt("com.bluechilli.flutteruploader.MAX_CONCURRENT_TASKS", DEFAULT_MAX_CONCURRENT_TASKS);
    ...
    public static int getConnectionTimeout(Context context) {
        try {
            ProviderInfo pi = context.getPackageManager().getProviderInfo(
                    ...
                    PackageManager.GET_META_DATA);
            Bundle bundle = pi.metaData;
            int max = bundle.getInt("com.bluechilli.flutteruploader.UPLOAD_CONNECTION_TIMEOUT_IN_SECONDS", DEFAULT_UPLOAD_CONNECTION_TIMEOUT);
